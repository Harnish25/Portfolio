...
export function ProjectCard({ title, year, type, summary, stack, accent }: ProjectCardProps) {
  return (
    ...
    >
      ...
      {/* Card inner */}
      <div className="relative z-10 m-px rounded-[calc(1rem-1px)] bg-black p-6 h-full">
        ...
        <div className="relative z-10 flex h-full flex-col gap-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.35em] text-cyan-400/80">{type}</p>
              <h3 className="mt-2 text-xl font-semibold text-white leading-snug">{title}</h3>
            </div>
          ...
          <p className="text-sm leading-7 text-white/55 flex-1">{summary}</p>
          ...
          <div className="flex items-end justify-between gap-3 pt-2 border-t border-white/[0.06]">
            <div className="flex flex-wrap gap-2">
              {stack.map((item) => (
                <span
                  key={item}
                  className="rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1 text-xs text-white/50 transition-colors duration-200 group-hover:border-cyan-400/20 group-hover:text-white/70"
                >
                  {item}
                </span>
              ))}
